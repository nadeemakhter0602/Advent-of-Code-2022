...
def traverse(rocks, resting_sand_units, sand_source, last_bottom):
    ...
    while len(stack) > 0:
        ...
        if down not in rocks and down not in resting_sand_units and down[1] != last_bottom:
            memo[down] = (i, j)
            stack.append(down)
        elif diagonal_left not in rocks and diagonal_left not in resting_sand_units and down[1] != last_bottom:
            memo[diagonal_left] = (i, j)
            stack.append(diagonal_left)
        elif diagonal_right not in rocks and diagonal_right not in resting_sand_units and down[1] != last_bottom:
